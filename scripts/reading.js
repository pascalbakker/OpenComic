...
var images = {}, imagesData = {}, imagesDataClip = {}, imagesPath = {}, imagesNum = 0, contentNum = 0,
    imagesNumLoad = 0, currentIndex = 1, imagesPosition = {}, imagesFullPosition = {}, prevImagesFullPosition = {},
    foldersPosition = {}, indexNum = 0, imagesDistribution = [], currentPageXY = {x: 0, y: 0},
    currentMousePosition = {pageX: 0, pageY: 0};
...
var previousScrollTop = 0, previousScrollHeight = 0, previousContentHeight = 0,
    ...
    if (readingViewIs('slide')) {
        ...
        if (stayInLineData.scrollTop === false)
            stayInLineData = {
                ...
                height: previousContentHeight,
                position: prevImagesFullPosition[currentIndex - 1][0],
                setTimeout: false
            };
...
var currentPageVisibility = 0, maxPageVisibility = 0, currentPageStart = true, readingDirection = true,
    disableOnScrollST = false;
...
function showNextComic(mode, animation = true, invert = false) {
    ...
    if (mode == 1) {
        ...
        if (_config.readingDelayComicSkip != 0) {
            ...
            skip.find('circle').css('animation-duration', _config.readingDelayComicSkip + 's').removeClass('a').delay(10).queue(function (next) {
                $(this).addClass('a');
                next();
            });
        }
...
var currentScale = 1, scalePrevData = {tranX: 0, tranX2: 0, tranY: 0, tranY2: 0, scale: 1, scrollTop: 0},
    originalRect = false, originalRectReadingBody = false, originalRect2 = false, originalRectReadingBody2 = false,
    haveZoom = false, currentZoomIndex = false, applyScaleST = false, zoomingIn = false, prevAnime = false;
...
function magnifyingGlassControlPrev() {
    if (magnifyingGlassPosition.mode !== false)
        magnifyingGlassControl(magnifyingGlassPosition.mode, {
            pageX: magnifyingGlassPosition.x,
            pageY: magnifyingGlassPosition.y,
            originalEvent: {touches: false}
        });
}
...
var hiddenContentLeft = false, hiddenBarHeader = false, hideContentDisableTransitionsST = false, hideContentST = false,
    hideContentRunningST = false, shownContentLeft = false, shownBarHeader = false;
...
function changePagesView(mode, value, save) {
    ...
    } else if (mode == 2) // Sets the margin of the pages
    {
        ...
        if (save) updateReadingPagesConfig('readingMargin', {
            margin: value,
            top: value,
            bottom: value,
            left: value,
    ...
    } else if (mode == 10) // Set horizontal margin of the pages
    {
        ...
        if (save) updateReadingPagesConfig('readingMargin', {
            margin: _config.readingMargin.margin,
            top: _config.readingMargin.top,
            bottom: _config.readingMargin.bottom,
            left: value,
            right: value
        });
        updateEbook(save);
    } else if (mode == 11) // Set vertical margin of the pages
    {
        ...
        if (save) updateReadingPagesConfig('readingMargin', {
            ...
            top: value,
            bottom: value,
            left: _config.readingMargin.left,
            right: _config.readingMargin.right
        });
        updateEbook(save);
    ...
    } else if (mode == 14) // Set horizontal margin of the horizontals pages
    {
        ...
        if (save) updateReadingPagesConfig('readingHorizontalsMargin', {
            margin: _config.readingHorizontalsMargin.margin,
            top: _config.readingHorizontalsMargin.top,
            bottom: _config.readingHorizontalsMargin.bottom,
            left: value,
            right: value
        });
        updateEbook(save);
    ...
    {
        updateReadingPagesConfig('readingImageClip', {
            top: _config.readingImageClip.top,
            bottom: _config.readingImageClip.bottom,
            left: value,
            right: value
        });
    ...
    } else if (mode == 17) // Clip vertical images
    {
        updateReadingPagesConfig('readingImageClip', {
            top: value,
            bottom: value,
            left: _config.readingImageClip.left,
            right: _config.readingImageClip.right
        });
...
async function generateEbookPages(end = false, reset = false, fast = false, imagePath = false, first = false) {
    ...
    if (hasGenerateEbookPages === 1) // Priorize last generateEbookPages request
    {
        ...
        for (let i = 0, len = ebookPages.pages.length; i < len; i++) {
            ...
            imagesData[index] = {
                width: ebookConfig.width,
                height: ebookConfig.height,
                aspectRatio: (ebookConfig.width / ebookConfig.height),
                name: page.name
            };
        ...
        setTimeout(function () {
            onScroll.call(template._contentRight().firstElementChild)
        }, 500);
...
var contentLeftRect = false, contentRightRect = false, barHeaderRect = false, touchevents = {
    active: false,
    start: false,
    distance: 0,
    scale: 0,
...
var touchTimeout, mouseout = {lens: false, body: false, window: false}, isMouseenter = {document: true},
    touchStart = false, magnifyingGlassOffset = false, readingCurrentPath = false, readingCurrentBookmarks = undefined,
    zoomMoveData = {}, magnifyingGlassScroll = {scrollTop: false, time: 0}, readingDragScroll = false,
    gamepadScroll = false, readingIsCanvas = false, readingIsEbook = false, readingFile = false, readingFileC = false,
    gamepadAxesNow = 0, scrollInStart = false, scrollInEnd = false, trackingCurrent = false;
...
async function read(path, index = 1, end = false, isCanvas = false, isEbook = false, imagePath = false) {
    images = {}, imagesData = {}, imagesDataClip = {}, imagesPath = {}, imagesNum = 0, contentNum = 0, imagesNumLoad = 0, currentIndex = index, foldersPosition = {}, currentScale = 1, currentZoomIndex = false, previousScrollTop = 0, scalePrevData = {
        tranX: 0,
        tranX2: 0,
        tranY: 0,
        tranY2: 0,
        scale: 1,
        scrollTop: 0
    }, originalRect = false, scrollInStart = false, scrollInEnd = false, prevChangeHeaderButtons = {}, trackingCurrent = false, pageRangeHistory = [];
    ...
    $(window).on('mouseup touchend', function (e) {
        ...
        if (readingDragScroll) {
            ...
            setTimeout(function () {
                reading.setReadingDragScroll(false)
            }, 10);
        }
...
module.exports = {
    ...
    imagesDataClip: function () {
        return imagesDataClip
    },
    scalePrevData: function () {
        return scalePrevData
    },
    goToPage: goToPage,
    goToImage: goToImage,
    goToFolder: goToFolder,
    goToIndex: function (v1, v2, v3, v4) {
        readingDirection = true;
        goToIndex(v1, v2, v3, v4)
    },
    ...
    saveReadingProgressA: function () {
        return saveReadingProgressA
    },
    ...
    currentPageVisibility: function () {
        return currentPageVisibility
    },
    ...
    currentReadingConfigKey: function () {
        return currentReadingConfigKey
    },
    onReading: function () {
        return onReading
    },
    calculateImagesDistribution: calculateImagesDistribution,
    imagesDistribution: function () {
    ...
    imagesPosition: function () {
        return imagesPosition
    },
    imagesFullPosition: function () {
        return imagesFullPosition
    },
    readingCurrentPath: function () {
        return readingCurrentPath
    },
    ...
    setShownContentLeft: function (value) {
        shownContentLeft = value
    },
    setShownBarHeader: function (value) {
    ...
    isEbook: function () {
        return readingIsEbook
    },
    isCanvas: function () {
        return readingIsCanvas
    },
